		...
		struct vm_area_list *vmas)
{
	...
	p.vmas = vmas; /* ibverbs may update flags for some vmas */
...
int dump_my_file(int lfd, u32 *id, int *type)
{
	...
	if (dump_one_file(&me, lfd, lfd, &fo, NULL, &e, NULL, NULL))
		return -1;
...
int dump_task_files_seized(struct parasite_ctl *ctl, struct pstree_item *item,
		struct parasite_drain_fd *dfds, struct vm_area_list *vmas)
{
	int *lfds = NULL;
